...
if myrank==0:
    print 'Memory usage before langreth declarations: %s (kb) % (kb)'% (resource.getrusage(resource.RUSAGE_SELF).ru_maxrss, process.memory_info().rss / 1024)
...
if myrank==0:
    print 'Memory usage after 5 langreth declarations: %s (kb) % (kb)'% (resource.getrusage(resource.RUSAGE_SELF).ru_maxrss, process.memory_info().rss / 1024)
...
if myrank==0:
    print 'Memory usage before sc loop. should be same as above: %s (kb) % (kb)'% (resource.getrusage(resource.RUSAGE_SELF).ru_maxrss, process.memory_info().rss / 1024)
...
for myiter in range(iter_selfconsistency):
    ...
    if myrank==0:
        print 'Memory usage before inversions: %s (kb) % (kb)'% (resource.getrusage(resource.RUSAGE_SELF).ru_maxrss, process.memory_info().rss / 1024)
    ...
    if myiter<iter_selfconsistency-1:
        ...
        for ik in range(kpp):
            ...
            if myrank==0:
                print 'Memory 1: %s (kb)'% resource.getrusage(resource.RUSAGE_SELF).ru_maxrss

            temp.zero(Nt, Ntau, Norbs)

            if myrank==0:
                print 'Memory 2: %s (kb)'% resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
            ...
            if myrank==0:
                print 'Memory 3: %s (kb)'% resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
            ...
            if myrank==0:
                print 'Memory 4: %s (kb)'% resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
            ...
            Gloc_proc.add(solve(temp, G0k, Nt, Ntau, dt, dtau, Norbs))

            if myrank==0:
                print 'Memory 5: %s (kb)'% resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
